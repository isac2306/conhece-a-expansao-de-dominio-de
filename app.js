...
function toggleReferencePanel() {
  if (!refs.referencePanel || !refs.referenceButton) {
    return;
  }

  const shouldShow = refs.referencePanel.classList.contains("hidden");
  refs.referencePanel.classList.toggle("hidden", !shouldShow);
  refs.referenceButton.textContent = shouldShow ? "Ocultar referencia" : "Ver referencia";
}
...
function updateGuidance(metrics, result, gestureReady) {
  ...
  setFeedbackItem(
    ...
    gestureReady
      ? "Gesto firme. O dominio pode abrir."
  ...
  setDebug("Gesto estabilizado. O dominio ativa so com o selo.");
}
...
function drawHandOverlay(landmarks, gestureScore) {
  ...
  context.strokeStyle = gestureStrong ? "rgba(255, 216, 128, 0.95)" : "rgba(143, 239, 255, 0.82)";
...
async function startHandTracking() {
  ...
  state.hands.onResults((results) => {
    ...
    const gestureReady = gestureScore > 0.72 && stabilityScore > 0.58;

    setMeter(refs.gestureMeter, gestureScore);
    setMeter(refs.stabilityMeter, stabilityScore);
    setMeter(refs.motionMeter, result.parts.framing);
  ...
  setDebug("Camera online. Monte o selo com indicador e medio cruzados.");
}
...
function initUi() {
  ...
  setPill(refs.motionStatus, "Modo gesto", "idle");
  refs.feedbackMotion.textContent = "Segure o gesto firme para ativar.";

  if (!state.recorder.supported) {
    refs.recordingText.textContent = "Clip recorder indisponivel neste navegador.";
  } else {
    refs.recordingText.textContent = "Modo leve ativo. O foco esta no sensor da mao.";
  }
}

refs.startButton.addEventListener("click", startExperience);
if (refs.referenceButton) {
  refs.referenceButton.addEventListener("click", toggleReferencePanel);
}
